# 🚗 Concesionario Sport - Simulador de Compra

Bienvenido a **Concesionario Sport**, un simulador de compra de autos desarrollado con JavaScript puro y almacenamiento local. Este proyecto tiene como objetivo simular el proceso de selección y compra de vehículos, ofreciendo una experiencia interactiva para el usuario.

---

## 🌐 Demo online

▶️ **[Probar el simulador en Netlify](https://simuladordecomprajavascript.netlify.app/)**

Podés ingresar y utilizar la app directamente desde cualquier dispositivo.
La demo se utiliza una base de datos de firebase

---

## 🧠 ¿Qué aprendí con este proyecto?

- Manipulación del DOM con JavaScript puro
- Uso de `localStorage` para almacenar datos
- Simulación de carrito de compras
- Renderizado dinámico de productos
- Validaciones básicas en formularios

---

## 🔐 Acceso al sistema

> Para acceder a la simulación de compra, usá las siguientes credenciales de prueba:

- **Usuario:** `ignacio`
- **Contraseña:** `1234`

*(Estas credenciales están pensadas para pruebas y demostraciones.)*

---

## 📸 Capturas

![Vista principal](./screenshots/vista-principal.png)
![Carrito](./screenshots/carrito.png)

---

## ⚙️ Funcionalidades principales

✅ Visualización de autos disponibles  
✅ Agregado de autos al carrito  
✅ Eliminación de autos del carrito  
✅ Cálculo del total de compra  
✅ Simulación de compra final  
✅ Persistencia de datos con `localStorage`

---

## 🚀 Tecnologías utilizadas

- HTML5
- CSS3
- JavaScript (ES6)
- `localStorage`

---

## 📁 Cómo usar este proyecto localmente

##🤝 Contribuciones
Si querés contribuir, podés:

Reportar issues o mejoras.

Crear un fork, aplicar tus mejoras y enviar un pull request.

Compartir el proyecto para llegar a más personas.


##📩 Contacto
Si querés contactarme por oportunidades o feedback, podés escribirme por:

LinkedIn: Ignacio Rojos

Email: ignaciorojos.dev@gmail.com

1️⃣ Cloná el repositorio:
```bash
git clone https://github.com/IgnacioRojos/Concesionario-Sport-simulador-de-compra.git


